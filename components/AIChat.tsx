...
const AIChat: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { id: '1', role: 'model', text: "What's good? I'm KickFlip. Ask me anything about drops, resale prices, or sizing advice." }
  ]);
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            >
              ...
              {/* Added Search Grounding Sources UI as required by guidelines */}
          ...
          <div className="flex justify-start">
             <div className="w-8 h-8 rounded-full bg-violet-600 flex items-center justify-center mr-2">
              ...
              <div className="bg-zinc-800 p-3 rounded-2xl rounded-tl-none border border-zinc-700 flex items-center">
                ...
                <span className="text-zinc-400 text-xs">Checking the database...</span>
              </div>
          </div>
        )}
      ...
      <div className="p-4 bg-zinc-900 border-t border-zinc-800">
        <div className="flex items-center bg-zinc-950 border border-zinc-800 rounded-full px-4 py-2 focus-within:border-violet-500 transition-colors">
          <input 
            ...
            placeholder="Ask about Jordan 1 sizing..."
            className="flex-1 bg-transparent border-none outline-none text-white text-sm placeholder-zinc-500"
          />
          <button 
            onClick={handleSend}
            disabled={isLoading || !input.trim()}
            className="ml-2 p-2 bg-violet-600 rounded-full text-white hover:bg-violet-500 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Send size={16} />
          </button>
        </div>
