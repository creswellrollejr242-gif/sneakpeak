...
const CreatorStudio: React.FC<CreatorStudioProps> = ({ onShowToast, sneakers }) => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `SneakPeak: ${selectedTemplate.overlay}`,
          text: `Just checked this out on SneakPeak: ${selectedSneaker?.name}`,
          url: window.location.href
        });
        onShowToast("Opened share menu", "success");
      } catch (err) {
      ...
      // Fallback
      onShowToast("Image saved to gallery! (Simulated)", "success");
    }
  };
  ...
  return (
    <div className="h-full overflow-y-auto pb-24 px-4 pt-4">
       <h1 className="text-2xl font-black italic tracking-tighter mb-1 text-white">CREATOR <span className="text-violet-500">STUDIO</span></h1>
       ...
       <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 mb-6 flex flex-col items-center justify-center relative overflow-hidden shadow-2xl aspect-[4/5]">
          ...
          <div className="absolute inset-0 bg-black">
             <img src={selectedSneaker.image} className="w-full h-full object-cover opacity-50 blur-xl scale-150" alt="Background" />
          </div>
