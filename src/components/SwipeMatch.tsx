...
const SwipeMatch: React.FC<SwipeMatchProps> = ({ onLike, sneakers }) => {
  ...
  // Shuffle sneakers on mount to show variety
  const shuffledSneakers = useMemo(() => {
    if (!sneakers) return [];
    return [...sneakers].sort(() => Math.random() - 0.5);
  }, [sneakers]);

  if (!shuffledSneakers || shuffledSneakers.length === 0) {
    return <div className="h-full flex items-center justify-center text-zinc-500">No kicks found.</div>;
  }

  const currentSneaker = shuffledSneakers[currentIndex % shuffledSneakers.length];
